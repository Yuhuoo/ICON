...
class ICON(pl.LightningModule):
    def __init__(self, cfg):
        ...
        self.pifu_keys = []
        self.refinepifu_keys = self.base_keys + [f"smpl_{feat_name}" for feat_name in self.feat_names] + ["voxel_verts", "voxel_faces", "pad_v_num", "pad_f_num"]
    ...
    def configure_optimizers(self):
        ...
        if self.cfg.net.prior_type == "refinepifu":
            optim_params_G.append({"params": self.netG.ve.parameters(), "lr": self.lr_G})
    ...
    def test_step(self, batch, batch_idx):
        ...
        image_inter = np.concatenate(self.tensor2image(512, inter[0]) + [smpl_F, smpl_B, image], axis=1)
        Image.fromarray((image_inter).astype(np.uint8)).save(osp.join(self.export_dir, f"{mesh_rot}_inter.png"))
        ...
        normal_consist = self.evaluator.calculate_normal_consist(osp.join(self.export_dir, f"{mesh_rot}_nc.png"))
    ...
    def test_single(self, batch):
        ...
        # icon ['normal_F', 'normal_B', 'image', 'T_normal_F', 'T_normal_B', 'smpl_verts', 'smpl_faces', 'smpl_sdf', 'smpl_cmap', 'smpl_norm', 'smpl_vis']
        # pamir ['image', 'normal_F', 'normal_B', 'T_normal_F', 'T_normal_B', 'voxel_verts', 'voxel_faces', 'pad_v_num', 'pad_f_num']
        # refinepifu ['image', 'normal_F', 'normal_B', 'T_normal_F', 'T_normal_B', 'voxel_verts', 'voxel_faces', 'pad_v_num', 'pad_f_num', 'smpl_verts', 'smpl_faces', 'smpl_sdf', ]
        
        with torch.no_grad():
            # icon: （1, 12, 128, 128) (1, 6, 512, 521)
